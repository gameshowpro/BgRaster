...
static class CliOptionCatalog
{
    ...
    [
        new("--config", "<path>", "string", "-", "Path to a TOML config file. If the path does not exist, BgRaster uses built-in defaults for that run and writes a seeded config template at that path after evaluating the run (including unchanged-skip runs).", "if omitted, searches for config.toml in this order: next to the executable, %ProgramData%\\BgInfo, %LocalAppData%\\BgInfo, %AppData%\\BgInfo; if none exist, starts from built-in defaults."),

...
static class CliOptionCatalog
{
    ...
    [
        ...
        new("--logo-x", "<dim>", "string", "[logo].x", "Logo center X.", "if omitted, defers to config.toml [logo].x; if missing there, uses [\"75vw\"]."),
        new("--logo-y", "<dim>", "string", "[logo].y", "Logo center Y.", "if omitted, defers to config.toml [logo].y; if missing there, uses [\"25vh\"]."),

...
internal static class UnitParser
{
    ...
    private static bool TryStripSuffix(ReadOnlySpan<char> span, string suffix, out ReadOnlySpan<char> remainder)
    {
    ...
    private static float ParseFloat(ReadOnlySpan<char> span, string original)
    {
        if (float.TryParse(span, NumberStyles.Float,

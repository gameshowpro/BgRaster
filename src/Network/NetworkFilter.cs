...
internal static class NetworkFilter
{
    internal static ImmutableArray<AdapterInfo> Apply(ImmutableArray<AdapterInfo> adapters, NetworkOptions options)
    {
        ...
        {
            HashSet<string> macs = new(options.RequireMacAddress.Select(NormalizeMac), StringComparer.OrdinalIgnoreCase);
    ...
    private static int GetAdapterTypeId(string typeShort) =>
        NetworkCollector.s_ianaShortToId.TryGetValue(typeShort, out int id) ? id : 0;

    private static bool TryParseAdapterType(string value, out int ianaId)
    {
    ...
    private static string NormalizeMac(string mac) => mac.Replace(":", "").Replace("-", "").Replace(".", "").ToUpperInvariant();

    private static bool MatchWildcard(string input, string pattern) =>
        FileSystemName.MatchesSimpleExpression(pattern, input);

    private static bool TryParseCidr(string cidr, out IPAddress? net, out int bits)
    {
        net = null;
        bits = -1;
        ...
        if (!IPAddress.TryParse(addrPart, out net))
        {
            return false;
        }

        if (!int.TryParse(bitsPart, out bits))
        {
            return false;
        }
    ...
    private static bool IsInSubnet(IPAddress addr, IPAddress net, int bits)
    {
        ...
        if (remainingBits > 0)
        {
            ...
            if ((a[byteCount] & mask) != (n[byteCount] & mask))
            {
                return false;
            }
        }
